...
- すべてのスクリーンショット用プレースホルダーは [images](images) 配下に配置している。
- ファイル名は差し替えしやすいように、英語で意味の分かる命名に統一している。
- Markdown 側のリンクはそのまま使えるため、同名ファイルで置き換えるだけで反映できる。

## 補足

- Day2 は Foundry + Fabric Data Agent + Teams の流れにフォーカスしている。
- 旧手順の TodoManagement、Azure Cosmos DB、MCP Toolkit 関連は本ラボの対象外である。
